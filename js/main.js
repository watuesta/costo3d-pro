...
function addPartRow(name="", weight="", time="") {
  ...
  row.className='part-row grid grid-cols-4 gap-1 items-center bg-white/5 p-2 rounded-xl border border-white/5';
  row.id=`part-${partsCount}`;
  // Solo un campo de tiempo (h.m)
  row.innerHTML=`
    <div class="col-span-1">
      <select class="part-name w-full bg-transparent text-xs outline-none font-medium px-1 uppercase text-blue-400 cursor-pointer">${nameOptions}</select>
    </div>
